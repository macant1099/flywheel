...
It also sharpens the car-club deep-dive that comes next. We now know:
...
- The featured sample research should include what these clubs do today (newsletter codes, forum threads, sponsorship programs) and where they break.
- The economic framing should lean on AARP's licensing model as the proof that trusted-audience monetization is real — rescaled for a club with 5,000–50,000 members.
...
1. **Exclusivity enforcement model.** Is LiveChannel's value prop best expressed as "the vendor gives you a real exclusive deal and the platform stops it from leaking," or "the vendor gives you what they give everyone, but the platform enforces that only your authenticated members can earn you the commission on it"? Different answers lead to different sales conversations.
2. **Data return to the source.** How granular does member-level data flow back to the org president? Full first-party purchase detail is maximum value but raises privacy/consent questions. A middle ground (aggregate cohort-level insights) may be the right first cut.
3. **Single-vertical MVP or multi-vertical?** The Flywheel breadth discipline says start with one vertical. Car clubs are the current lead, but the landscape also surfaced alumni and consumer-hobby-membership segments as similarly underserved. Worth validating car clubs as THE right first vertical before the deep-dive locks it in.

## Where this lives

Entry files live on the portal at `/sources/` and in this repository at `research/sources/entries/` (to be mirrored — the portal currently holds the canonical first-sweep copy). This landscape digest is the rolling state-of-the-landscape view; it refreshes as new signals arrive and as Phase 2 entries are added.
